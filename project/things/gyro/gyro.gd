...
func _physics_process(delta):
	$Plane.rotation_degrees = Vector3(move_toward($Plane.rotation_degrees.x, rotationobjective.x, 0.5)/clamp($Plane.global_position.distance_to(ball.global_position),1,100), 0, move_toward($Plane.rotation_degrees.z, rotationobjective.z, 0.5)/clamp($Plane.global_position.distance_to(ball.global_position),1,100))
