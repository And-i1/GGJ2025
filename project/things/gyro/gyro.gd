...
func _ready() -> void:
	...
	if JavaScriptBridge.eval("typeof DeviceOrientationEvent.requestPermission === 'function'"):
		$CenterContainer/Label.text += "Permission needed "
		dev_orientation.requestPermission().then(_permission_cb)
	else:
		$CenterContainer/Label.text += "No permission needed "
...
func asked_for_permission(args) -> void:
	...
	if permission_state == "granted":
		$CenterContainer/Label.text += "Permission granted for DeviceOrientation "
		JavaScriptBridge.get_interface("window").ondeviceorientation = _dev_orientation
	else:
		$CenterContainer/Label.text += "Permission not granted for DeviceOrientation "
